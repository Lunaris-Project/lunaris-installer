...
func (m Model) installationView() string {
	...
	if m.awaitingPassword {
		...
		visibilityToggle := ""
		if m.passwordVisible {
			visibilityToggle = DimStyle.Render("[Tab] Hide password")
		} else {
	...
	if m.awaitingPassword {
		...
		case tea.KeyEnter:
			...
			if len(m.passwordInput) > 0 {
				...
				m.passwordInput = ""     // Clear password after submission
				m.passwordVisible = true // Reset to visible for next time

				// Return a command to use the password
				return m, func() tea.Msg {
					// Set the password for sudo operations
					if m.aurHelper != nil {
					...
					return InstallProgressMsg{
						...
						Current:  "Continuing installation with sudo access...",
					}
				}
			}
		case tea.KeyTab:
